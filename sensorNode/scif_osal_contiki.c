...
PROCESS(ready_interrupt, "OSAL Ready Interrupt");
PROCESS_THREAD(ready_interrupt, ev, data)
{
	PROCESS_BEGIN( );

	LOG_DBG("Ready interrupt process starting\n");
	while(1) {
...
PROCESS_THREAD(alert_interrupt, ev, data)
{
	...
	LOG_DBG("Alert_interrupt process starting\n");
	while(1) {
		PROCESS_YIELD_UNTIL(ev == PROCESS_EVENT_POLL);

		LOG_DBG("Received alert interrupt - reading/posting sensor data\n");
...
int sensor_aux_init( )
{
	...
	LOG_INFO("Turning on SCS domain\n");
    scifOsalEnableAuxDomainAccess( );


	LOG_INFO("Starting SCS ready and alert interrupt responders\n");
	// start the interrupt threads
	process_start(&ready_interrupt, NULL);
	process_start(&alert_interrupt, NULL);

	LOG_DBG("Enabling interrupt handlers\n");
	osalEnableCtrlReadyInt();
	osalEnableTaskAlertInt();

	LOG_DBG("NVIC ready status: %lu\n", HWREG(NVIC_OFFSET(INT_SCIF_CTRL_READY)));
	LOG_DBG("NVIC alert status: %lu\n", HWREG(NVIC_OFFSET(INT_SCIF_TASK_ALERT)));

	LOG_DBG("Calling scifScsDriverSetup\n");
	int rc = scifInit(&scifScsDriverSetup);
	if (rc != SCIF_SUCCESS) {
		LOG_ERR("Error during scifInit - %d\r\n", rc);
		return -1;
	}

	LOG_DBG("Calling scifStartTaskNbl\n");
	rc = scifStartTasksNbl(BV(SCIF_SCS_READ_DATA_TASK_ID));
	if (rc != SCIF_SUCCESS) {
		LOG_ERR("Error during scif control start: %d\r\n", rc);
	} else {
		LOG_ERR("Scif control start SUCCESSFUL\r\n");
	}

	LOG_DBG("Enabling timer1 for AUX module\n");
