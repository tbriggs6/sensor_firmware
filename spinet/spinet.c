			    ...
			    int *xfer_len, int *curr_reg)
{
  ...
  if (spi_in[3] != 0xff) {
   ...
   // start receiving a packet from the raspberry pi
    ...
    LOG_DBG("Started command 20 - len = %d\n", spi_len);
...
slv_state_t process_finrcv(const char *spi_in, int xfer_len)
{
  if (!((xfer_len > 0) && (xfer_len < UIP_BUFSIZE))) {
      ...
      LOG_DBG("*SKIPPING* tcpip input %d : %x %x %x %x\n", xfer_len, spi_in[0], spi_in[1], spi_in[2], spi_in[3]);
...
PROCESS_THREAD(spislv_ctrl, ev, data)
{
  ...
  ti_lib_gpio_clear_dio(27);
  IOCPinTypeGpioOutput(27);
  GPIO_setOutputEnableDio(27, GPIO_OUTPUT_ENABLE);

  ti_lib_gpio_clear_dio(26);
  IOCPinTypeGpioOutput(26);
  ...
  while (1)
    {
	  ...
	  switch(slv_state) {
	    case STATE_IDLE:
	      next_state = process_command(spi_in, spi_out, sizeof(spi_out),
					   (int *) &xfer_len, (int *) &curr_reg);
	      if (next_state == STATE_FIN_RCV) {
	          etimer_set(&dbg_timer, CLOCK_SECOND);
	          need_to_wait = 1;
	      }
	      break;
	    ...
	    case STATE_FIN_RDREG:
	      ...
	      // read from PI, send OTA
	    case STATE_FIN_RCV: {
	      next_state = process_finrcv(spi_in, xfer_len);
	      etimer_set(&dbg_timer, CLOCK_SECOND);

	      // can't wait in a case statement
	      need_to_wait = 1;
	    }
	      break;
	  ...
	  if (need_to_wait) {
	      LOG_DBG("Waiting for TX to finish. %d\n", uip_len);
	      PROCESS_WAIT_EVENT_UNTIL(etimer_expired(&dbg_timer));
	      LOG_DBG("done waiting\n");
	      //PROCESS_WAIT_UNTIL(uip_len == 0);
	  ...
	  // and now that the DMA is reloaded, signal
	  toggle_spi_intr( );
